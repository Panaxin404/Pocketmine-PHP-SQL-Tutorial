...
class Main{
    ...
    public static function getDB(){
        $db = new \mysqli("ip", "name", "password", "databasename", "port");
    ...
    public static function registerPlayerInDB(string $name){
        if (self::checkIfPlayerIsRegisteredInTheDatabase() === null){
        $db = self::getDB();
        $db->query("INSERT INTO playerinfo $name, 1, 0");
        $db->close;
        }
    }
    ...
    public static function addPlayerLevel(string $name){
        $currentLevel = self::getPlayerLevel();
